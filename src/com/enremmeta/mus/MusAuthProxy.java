...
public abstract class MusAuthProxy extends Base {
	...
	public void init(Engine context) throws IOException, UnknownHostException {
		...
		this.logger.info("Connected to mysql server at " + this.mysqlHost + ":"
				+ this.mysqlPort);
		this.mysqlIn = new BufferedInputStream(
				this.mysqlSocket.getInputStream(), 16384);
		this.mysqlOut = this.mysqlSocket.getOutputStream();

		mysqlUser = JMP.config.getProperty("mus_mysql_user");
		mysqlPass = JMP.config.getProperty("mus_mysql_pass");

	}

    public void read_handshakex(Engine context) throws IOException {
	...
	public void read_handshake(Engine context) throws IOException {
		...
		context.handshake.removeCapabilityFlag(Flags.CLIENT_SSL);
		context.handshake.removeCapabilityFlag(Flags.CLIENT_LOCAL_FILES);
		// chal.removeCapabilityFlag(Flags.CLIENT_SECURE_CONNECTION);

		context.handshake.setCapabilityFlag(Flags.CLIENT_PLUGIN_AUTH);
		context.handshake.authPluginName = "mysql_clear_password";

		System.out.println(context.handshake.toString());
		// Set the default result set creation to the server's character set
		ResultSet.characterSet =context.handshake.characterSet;
	...
	public void read_auth(Engine context) throws IOException {
		...
		try {
			String u = context.authReply.username;
			String p = context.authReply.authResponse;

			for (int i = 0; i < p.length(); i++) {
				System.out.print((byte) p.charAt(i));
			}
			...
			context.authReply.username = this.mysqlUser;
			context.authReply.authResponse = this.mysqlPass;
			context.buffer.add(context.authReply.toPacket());
		} catch (MusException me) {
			this.logger.fatal("Cannot authenticate");
			context.halt();
			return;
		}
	...
	public void read_query_result(Engine context) throws IOException {
		...
		default:
			context.buffer = Packet.read_full_result_set(this.mysqlIn,
